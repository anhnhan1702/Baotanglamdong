var vm = new Vue({
    ...
    methods: {
        ...
        getdatabosuutaps() {
            const self = this;
            axios.get("/danh-muc/bosuutaps").then(function (response) {
                self.databosuutaps = response.data;
        ...
        galleryhienvat(data) {
            const self = this;
            window.location.href = "/gallery-hien-vat?id=" + data.id +"&table=bosuutaps";
        }, 
        ...
        submitform() {
            ...
            if (this.statusForm == "insert") {
                axios.post("/save-bo-suu-tap", self.dataForm.data).then(function (response) {
                    self.thongbaothanhcong('Lưu thành công')
                    self.loadData();
                    self.getdatabosuutaps();
                })
                    .catch(error => {
        ...
        doAlertDelete(data, row, tr, target) {
            ...
            this.$confirm('Thao tác này không thể quay lại, bạn chắc chắn tiếp tục?', 'Cảnh báo', {
                ...
                axios.delete("/delete-bo-suu-tap?id=" + data.id)
                    .then(function (response) {
                        self.loadData();
                    })
