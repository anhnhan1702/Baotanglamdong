var vm = new Vue({
    ...
    mounted: function () {
        ...
        axios.get("/danh-muc/chucvus").then(function (response) {
            self.nhomquyen = response.data;
        })
        this.getdatanhomquyens();
    ...
    methods: {
        ...
        getdatanhomquyens() {
            const self = this;
            axios.get("/danh-muc/chucvus").then(function (response) {
                self.datanhomquyens = response.data;
        ...
        // view hiện vật
        gallerynhomquyen(data) {
            const self = this;
            window.location.href = "/gallery-hien-vat?id=" + data.id +"&table=nhomquyens";
        }, 
       
        
        // end hiện vật 
        saveform() {
            this.tieude = 'Thêm mới nhóm quyền'
        ...
        submitform() {
            ...
            if (this.statusForm == "insert") {
                axios.post("/save-nhomquyen", this.dataForm.data
                ).then(function (response) {
                    self.thongbaothanhcong('Lưu thành công')
                    self.loadData();
                    self.getdatanhomquyens();
                })
                    .catch(error => {
        ...
        doAlertDelete(data, row, tr, target) {
            ...
            this.$confirm('Thao tác này không thể quay lại, bạn chắc chắn tiếp tục?', 'Cảnh báo', {
                ...
                axios.delete("/delete-nhomquyen?id=" + data.id)
                    .then(function (response) {
                        self.loadData();
                    })
