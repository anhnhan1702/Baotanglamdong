Vue.component('modalloaitruong6', {
    data: function () {
        return {
            dataTuyentruyen: []
        }
    ...
    methods: {
        ...
        giatritruyentruyen(tuyentruyen) {
            var giatri = {};
            // Thêm nội dung sẽ inser vào data
            giatri['noidung'] = tuyentruyen.noidung;
            giatri['dientich'] = tuyentruyen.dientich;
            // end
            const self = this;
                        ...
                        for (var i = 0; i < 11; i++) {
                            if (self.dataTuyentruyen[i]) {
                                if (self.dataTuyentruyen[i].giatri != null) {
                                    if (self.dataTuyentruyen[i].giatri['noidung']) {
                                        self.dataTuyentruyen[i].noidung = self.dataTuyentruyen[i].giatri['noidung']
                                    }
                                    if (self.dataTuyentruyen[i].giatri['dientich']) {
                                        self.dataTuyentruyen[i].dientich = self.dataTuyentruyen[i].giatri['dientich']
                                    }
                                }
                            }
                        }
                ...
                <table class="table table-report z-10 ">
                    <thead>
                        <tr class="">
                            ...
                            <th class="text-center  ">Kiểm chứng</th>
                    ...
                    <tbody>
                        <tr class="intro-x testthu" v-for="(i,index) in dataTuyentruyen">
                            ...
                            <td class=" repon_modal " data-lable="Diện tích">
                                <input  @change="giatritruyentruyen(i)" v-model="i.dientich" :disabled="(i.sudung == 1 ? true : false) || (trangthai == 2 ? true :false) " type="text" step="0.01" class="px-3 py-2 border-2 border-gray-300 rounded-lg" />
                            </td>
                            ...
                            <td class=" repon_modal " data-lable="Chức năng">
                                <div class="">
