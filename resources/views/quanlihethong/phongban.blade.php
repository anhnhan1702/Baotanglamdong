            ...
            <div class=" w-full  overflow-auto " >
                ...
                <div class="grid grid-cols-12 ">
                    <div class="col-span-12">
                        <label for="name"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tên phòng ban:</label>
                    ...
                    <div class="col-span-12 mt-4">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Thành viên:</label>
                            <el-select class="w-full !rounded-lg" v-model="dataForm.users" multiple filterable allow-create default-first-option placeholder="Chọn thành viên">
                                <el-option v-for="item in listUser" :key="item.id" :label="item.name +' - '+ item.chucvu" :value="item.id">
                                </el-option>
                          </el-select>
                        </span>
                    </div>
...
var vm = new Vue({
    ...
    methods: {
        ...
        getUsers() {
            const self = this;
            axios.get("/ajax-nguoi-dung").then(function(response) {

                self.listUser = response.data.data;
        ...
        saveform() {
            this.tieude = 'Thêm mới phòng ban'
            this.dataForm.data.name = '';
            this.dataForm.data.users = [];
        ...
        submitform() {
            ...
            if (this.statusForm == "insert") {
                axios.post("/save-phong-ban", self.dataForm.data).then(function(response) {
        ...
        doAlertEdit(data) {
            ...
            this.tieude = 'Sửa thông tin phòng ban'
            this.dataForm.data.name = data.name;
            this.dataForm.data.users = [];

            this.dataForm.data.users = data.users;
        ...
        doAlertDelete(data, row, tr, target) {
            ...
            this.$confirm('Thao tác này không thể quay lại, bạn chắc chắn tiếp tục?', 'Cảnh báo', {
                ...
                axios.delete("/delete-phong-ban?id=" + data.id)
                    .then(function(response) {
                        self.loadData();
