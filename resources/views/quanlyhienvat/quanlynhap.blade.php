...
<div id="demo" class="py-6  h-full ">
                ...
                <div class="col-span-12  bg-white mr-8 mt-4">
                    <table id="customers">
                        <tr class="">
                            ...
                            <th class="w-3/12">Hiện vật</th>
                            <th class="w-1/12">Số lượng</th>
                            <th class="w-2/12">Kích thước</th>
                        ...
                        <tr v-for="(i,key) in  dataForm.danhmuchienvat">
                            <td>
                                ...
                                <treeselect v-model="i.vitri_id" v-on:select="chonvitrihienvat" :multiple="false"
                                    :options="danhsachvitrikho" placeholder="Chọn vị trí" :normalizer="normalizer" />
                            </td>
                            <td>
                                @{{i.hienvat}}
                            </td>
        ...
        <div class=" sm:h-11/12 h-4/5 p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white">
            ...
            <div id="hiddenScroll" class="px-4 py-6 w-full grid grid-cols-12 overflow-y-auto " style="height:90%">
                <div id="printMe" class=" col-span-12 grid grid-cols-12">
                    ...
                    <div class="col-span-12  text-center sm:text-3xl text-base py-2 mt-2 font-medium">
                       PHIẾU NHẬP HIỆN VẬT
                    </div>
                    <div class="sm:col-span-12 col-span-12 ">
                        ...
                        <p class="mt-1">
                            Tổng số hiện vật nhập: @{{danhsachhienvat_nhap.length}}<br>
                            Số hiện vật bằng chữ: @{{soluong_nhap}}<br>
    ...
    <div class="mx-auto max-w-8xl flex">
        ...
        <h1 class="text-base sm:text-xl mt-1 text-gray-900 dark:text-light">&nbsp; Nhập hiện vật</h1>
    </div>
                                    ...
                                    <template>
                                        ...
                                        <el-table-column align="center" width="270">
                                            ...
                                            <template slot-scope="scope">
                                                ...
                                                <el-tooltip class="item" v-if="scope.row.trangthai==0" effect="dark"
                                                ...
                                                <el-tooltip class="item" effect="dark" content="Xuất phiếu nhập"
                                                    placement="top-start">
...
var vm = new Vue({
            ...
            if (this.checkdelete != 0) {
                ...
                for (var i = 0; i < self.danhsachhienvat.length; i++) {
                    ...
                    if (check == true) {
                        ...
                        data['soluong'] = self.danhsachhienvat[i]['soluong'];
                        data['hienvat'] = self.danhsachhienvat[i]['name'];
                        for (let index = 0; index < this.hienvatdaco.length; index++) {
                            if (this.hienvatdaco[index]['hienvat_id'] == data['hienvat_id']) {
                                data['vitri_id'] = self.hienvatdaco[index]['vitri_id'];
                            }
                        }
    ...
    methods: {
        ...
        // end chuyển chuỗi số thành chữ




        chonvitrihienvat: function(infovitri) {
            
            const self = this;
            var datavitrikhonew = [];
            for (let index = 0; index < self.danhsachvitrikho.length; index++) {
                var children = [];

                if (self.danhsachvitrikho[index].children.length > 0) {

                    for (let i = 0; i < self.danhsachvitrikho[index].children.length; i++) {
                        if (infovitri.id == self.danhsachvitrikho[index].children[i].id) {
                            var vitridachon = [];
                            vitridachon['id'] = self.danhsachvitrikho[index].children[i].id;
                            vitridachon['hienvat_id'] = self.danhsachvitrikho[index].children[i].hienvat_id;
                            vitridachon['name'] = self.danhsachvitrikho[index].children[i].name;
                            vitridachon['trangthai'] = self.danhsachvitrikho[index].children[i].trangthai;
                            vitridachon['isDisabled'] = true;
                            children.push(vitridachon);
                        } else {
                            children.push(self.danhsachvitrikho[index].children[i]);
                        }

                    }
                }
                var capnhatvitri = [];
                capnhatvitri['name'] = self.danhsachvitrikho[index].name;
                capnhatvitri['children'] = children;
                datavitrikhonew.push(capnhatvitri)
            }
            self.danhsachvitrikho = datavitrikhonew;
        },
        duyet(row) {
            const self = this;
            axios.post("/duyet-phieu", row).then(function(response) {
                    if (response.data == 0) {
                        ...
                        self.thongbaothatbai("Vị trí không thể trống");
                    }
                })
                .catch(error => {
                    self.thongbaothatbai(error);
        ...
        openmodal_2(data) {
            ...
            const self = this
            self.soluong_nhap = ''
            self.soluong_nhap += this.docso(data.danhmuchienvat.length)
            this.danhsachhienvat_nhap = data.danhmuchienvat
        ...
        saveform() {
            ...
            this.dataForm.data.loaixuatnhap = 2;
        ...
        doAlertEdit(data) {
            ...
            this.dataForm.data.danhmucxuat = data.danhmucxuat;
